...
class Faturamentos extends BaseController
{
    ...
    public function locacoes()
    {
        ...
        if ($dataInicio && $dataFim) {
            ...
            foreach ($locacoes as $locacao) {
                $valorTotal += $this->parseMoney($locacao->valor_total);
            }
    ...
    /** helper no mesmo controller (privado) */
    private function parseMoney($value): float
    {
        ...
        // se vem no formato "1.234,56" (pt-BR)
        if (strpos((string)$value, ',') !== false) {
            $v = str_replace('.', '', $value); // remove separador de milhar
            $v = str_replace(',', '.', $v);    // substitui decimal
            return (float) $v;
        }

        // se já for "1234.56" ou numérico
        return (float) $value;
    }
}
