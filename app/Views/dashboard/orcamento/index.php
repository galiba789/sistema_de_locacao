            ...
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-2">
                    <a href="<?= base_url('orcamento/cadastrar') ?>">
                        <button type="button" class="btn btn-success"><i class="fa-solid fa-pen"></i> Nova Orçamento</button>
                    </a>
                </div>
            </div>
        </div>
                ...
                <tbody id="tabela-locacoes">
                    <?php foreach ($orcamentos as $orcamento): ?>
                        <tr>
                            ...
                            <td>
                                <a href="clientes/historico/<?= $orcamento['cliente_id'] ?? '#' ?>">
                                    <?= isset($orcamento['cliente_id']) && !empty($orcamento['cliente_nome']) ? $orcamento['cliente_nome'] : 'Desconhecido' ?>
                                </a>
                            </td>
                            ...
                            <td>
                                ...
                                <?php elseif ($orcamento['situacao'] == 3): ?>
                                    <span class="btn btn-warning">Atrasado</span>
                            ...
                            <td>
                                <?php if ($orcamento['pagamento'] == 0): ?>
                                    <a href="<?= base_url('locacoes/pagamento/') . $orcamento['id'] ?>"><span class="btn btn-warning">Pendente</span></a>
                                <?php elseif ($orcamento['pagamento'] == 1): ?>
                                    <a href="<?= base_url('locacoes/pagamento/') . $orcamento['id'] ?>"><span class="btn btn-success">Pago</span></a>
                                <?php endif; ?>
                            </td>
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    let rows = data.map(orcamento => {
                        ...
                        let valorTotal = orcamento.valor_total ? parseFloat(orcamento.valor_total).toLocaleString('pt-BR', {
                            style: 'currency',
                            currency: 'BRL'
                        }) : 'R$ 0,00';
